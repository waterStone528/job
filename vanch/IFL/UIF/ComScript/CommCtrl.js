...
LoadCSS("ComStyle");
function GetVer() {
    var SP = location.search.indexOf("V=") + 2;
    var EP = location.search.indexOf("&", SP);
    var Ver = location.search.substring(SP, EP);
    if (EP == -1) { Ver = location.search.substring(SP, location.search.length); }
    return Ver;
}
...
function LoadCSS(CssName, Floder) {
    if (!Floder) { Floder = ""; } else { Floder = Floder + "/"; }
    var css = document.createElement("link");
    var Head = document.head || document.getElementsByTagName('head')[0];
    css.setAttribute("href", Floder + CssName + ".css?v= " + SysUIVer);
...
function Location(PageName, ModFloder, Ext, Param) {
    ///<summary>页面跳转</summary>
    ///<param name="PageName">文件名</param><param name="ModFloder">文件所在的目录</param>
    ///<param name="Ext">文件扩展名</param><param name="Param">参数</param>
    if (!ModFloder) { ModFloder = ""; } else { ModFloder = ModFloder + "/"; }
    if (!Ext) { Ext = "html"; }
    if (!Param) { Param = "?V=" + SysUIVer; } else { Param = "?" + Param + "&V=" + SysUIVer }
    location.href = ModFloder + PageName + "." + Ext + Param;
}
...
var PublicSU = {
    ...
    UID: {
        LoginAction: function (UserName, UserSN) {
            ///<summary>用户登陆后执行的事件</summary><param name="UserName">用户姓名</param><param name="UserSN">用户编号</param>
            PublicSU.UIP.UserSN = UserSN;
            ...
            TopBarButtons.querySelectorAll(".TopBarButtonsText")[2].setAttribute("data-islogin", "1");
            TopBarButtons.querySelectorAll(".TopBarButtonsText")[4].setAttribute("class", "TopBarButtonsText");
...
var Comm = {
    ...
    UID: {
        logout: function (LogoutTS) {
            ///<summary>用户退出</summary>
            ///<param name="LogoutTS">1:登录超时 ;  2:未登录 </param>
            if (LogoutTS == 2) { return; };
            var NewItemTemplate = logouti.children[0].cloneNode(true);
            if (LogoutTS == 1) { document.querySelector('.MaskLayer').style.display = 'block'; NewItemTemplate.querySelectorAll('label')[0].innerHTML = '登录超时 ...'; }
            if (LogoutTS == 0) { document.querySelector('.MaskLayer').style.display = 'block'; NewItemTemplate.querySelectorAll('label')[0].innerHTML = '您尚未登录 ...'; }
