...
var M0601 = {
    ...
    STR: {
        ...
        EVINIT: function (Num) {
            ...
            var busCode = "M0601INIT";
            var data = "busCode=" + busCode + "&pageSize=" + Num;
            var ajaxObj = new AJAXC();
            ajaxObj.data = data;
            ajaxObj.success = function (res) {
                var sumAmount = { C1: 0, C2: 0, C3: 0, C4: 0, C5: 0, C6: 0 };

                var resObj = JSON.parse(res);
                for (var i in resObj) {
                    M0601.UID.M060101(resObj[i].userSN, resObj[i].LoginName, resObj[i].name.trim(), resObj[i].vipLevel, resObj[i].topUpAmount == null ? 0 : resObj[i].topUpAmount, resObj[i].consumptionAmount == null ? "0" : resObj[i].consumptionAmount, resObj[i].balanceAmount == null ? 0 : resObj[i].balanceAmount, resObj[i].needPayBillAmount, resObj[i].needPayAmount == null ? "0" : resObj[i].needPayAmount, resObj[i].overDueAmount == null ? "0" : resObj[i].overDueAmount);

                    sumAmount.C1 += resObj[i].topUpAmount == null ? 0 : resObj[i].topUpAmount;
                    sumAmount.C2 += resObj[i].consumptionAmount == null ? 0 : resObj[i].consumptionAmount;
                    sumAmount.C3 += resObj[i].balanceAmount == null ? 0 : resObj[i].balanceAmount;
                    sumAmount.C4 += resObj[i].needPayBillAmount == null ? 0 : resObj[i].needPayBillAmount;
                    sumAmount.C5 += resObj[i].needPayAmount == null ? 0 : resObj[i].needPayAmount;
                    sumAmount.C6 += resObj[i].overDueAmount == null ? 0 : resObj[i].overDueAmount;
                }

                M0601.UID.M060105(sumAmount);

                var ifOverdue = resObj.length < Num ? true : false;
                M0601.UID.M060102(ifOverdue, resObj.length);
            }
            ajaxObj.start();
        },
        //加载更多数据
        EVM060101: function (Form, Num, Sort) {
            ...
            var busCode = "M060101";
            var data = "busCode=" + busCode + "&sortStr=" + L1FTParam + "&pageFrom=" + Form + "&pageSize=" + Num;
            var ajaxObj = new AJAXC();
            ajaxObj.data = data;
            ajaxObj.success = function (res) {
                var sumAmount = { C1: 0, C2: 0, C3: 0, C4: 0, C5: 0, C6: 0 };
                ...
                var ifOverdue = resObj.length < Num ? false : true;
                M0601.UID.M060102(ifOverdue, resObj.length);
            }
            ajaxObj.start();
        },
        EVM060102: function (L1FTParam, Num) {//主表过滤事件触发,L1FTParam为条件数组，格式为L1FTParam[0],L1FTParam[1].....
            //console.log(Sort);
            //M0601.UID.M060103();
            //setTimeout("for(var i=0;i<" + Num + ";i++){M0601.UID.M060101(\"U005321\", \"Admin1\", \"姓名1\", 6, 10000, 8000, 2000, 3, 3500, 1000);};M0601.UID.M060102(false," + Num + ");", 1000);
            //var Data = { C1: 10000, C2: 20000, C3: 30000, C4: 40000, C5: 50000, C6: 60000 };
            ...
            var busCode = "M060102";
            var data = "busCode=" + busCode + "&sortStr=" + L1FTParam + "&pageSize=" + Num;
            var ajaxObj = new AJAXC();
            ajaxObj.data = data;
            ajaxObj.success = function (res) {
                M0601.UID.M060103();
        ...
        EVM060103: function (UserSN) {//应收账单
            ...
            ajaxObj.success = function (res) {
                var resObj = JSON.parse(res);
                for (var i in resObj) {
                    var dataFt = M0601.STR.BillInfoBdToFt(resObj[i]);
                    M0601.UID.M060104(dataFt)
                }
            }
            ajaxObj.start();
        },
        ...
        //后-前 应收账单
        BillInfoBdToFt: function (dataBd) {
            var dataFt = {};

            dataFt.C1 = dataBd.billSN;
            dataFt.C2 = dataBd.businessSN.trim();
            dataFt.C3 = dataBd.billType.trim();
            dataFt.C4 = dataBd.MoneyAmount;
            dataFt.C5 = BdDateStrFormate(dataBd.generateDate);
            dataFt.C6 = dataBd.lateFee;
            dataFt.C7 = BdDateStrFormate(dataBd.needPayDate);
            dataFt.C8 = dataBd.MoneyAmount + dataBd.lateFee;
