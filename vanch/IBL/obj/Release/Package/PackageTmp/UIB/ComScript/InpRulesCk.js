...
var IRC = {
    ...
    LoginNameCk: function (InpObj, InpValue) {
        if (InpValue.length == 3) {
            ...
            this.ErrText(InpObj, "必须输入三位数的工号");
            Pass = false;
        }
        return Pass;
    },
    ...
    CheckOldPwd: function (InpObj, InpValue) {
        if (InpValue.length > 0) {
            this.InputErrBorder(InpObj.parentNode, false);
            Pass = true;
        }
        else {
            this.ErrText(InpObj, "请输入原密码");
            Pass = false;
        }
        return Pass;
    },
    CheckNewPwd: function (InpObj, InpValue) {
        if (InpValue.length > 0) {
            this.InputErrBorder(InpObj.parentNode, false);
            Pass = true;
        }
        else {
            this.ErrText(InpObj, "密码不能为空");
            Pass = false;
        }
        return Pass;
    },
    ComparePwd: function (InpObj, InpValue, ReInpObj, ReInpValue) {
        if (ReInpValue == InpValue) {
            this.InputErrBorder(ReInpObj.parentNode, false);
            Pass = true;
        }
        else if (InpValue.length > 0 && ReInpValue.length == 0) {
            this.ErrText(ReInpObj, "请再次输入新密码");
            Pass = false;
        }
        else if (ReInpValue.length > 0 && ReInpValue != InpValue) {
            this.ErrText(ReInpObj, "两次密码不一致");
            Pass = false;
        }
        return Pass;
    },
    ...
    ErrTip: function (InpObj, ErrTxt) {
        ...
        var ErrBorderObjPadding = parseInt(ErrBorderObj.currentStyle.padding.replace("px", ""));
        ErrDiv.innerHTML = "<div class=\"PmpTxt\" style=\"background-color:#000; position:absolute;left:" + -ErrBorderObjPadding + "px;width:" + (InpObj.clientWidth + 0) + "px;height:" + (InpObj.clientHeight) + "px;line-height:" + (InpObj.clientHeight - ErrBorderObjPadding) + "px" + "\">"+ErrTxt+"</div>";
